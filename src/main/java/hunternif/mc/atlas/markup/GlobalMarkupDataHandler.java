 ...
 * @author Hunternif
 */
public class GlobalMarkupDataHandler {
	private static final String DATA_KEY = "aAtlasGlobalMarkup";

	private GlobalMarkupData data;

	@SubscribeEvent(priority=EventPriority.HIGHEST)
	public void onWorldLoad(WorldEvent.Load event) {
		if (!event.getWorld().isRemote && event.getWorld().provider.getDimension() == 0) {
			data = (GlobalMarkupData) event.getWorld().loadData(GlobalMarkupData.class, DATA_KEY);
			if (data == null) {
				data = new GlobalMarkupData(DATA_KEY);
				data.markDirty();
	...
	public GlobalMarkupData getData() {
